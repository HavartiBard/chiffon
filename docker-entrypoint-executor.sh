...
if [ "$EXECUTION_MODE" = "adhoc" ]; then
    ...
    /app/.venv/bin/python3 -m chiffon.cli run-once --project "$PROJECT" --repo "$REPO_PATH" --use-llm
    exit $?
fi
...
CRON_JOB="$CRON_SCHEDULE /app/.venv/bin/python3 -m chiffon.cli run-once --project $PROJECT --repo $REPO_PATH --use-llm >> /var/log/chiffon-cron.log 2>&1"
