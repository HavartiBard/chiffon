...
async def test_subscription_lifecycle(
    manager: WebSocketManager, dummy_server: DummySocketServer
) -> None:
    ...
    await manager.handle_unsubscribe(
        "sid-1", {"subscription_id": list(manager.subscription_to_plan)[0]}
    )
...
@pytest.mark.asyncio
async def test_broadcast_emits_plan_event(
    manager: WebSocketManager, dummy_server: DummySocketServer
) -> None:
